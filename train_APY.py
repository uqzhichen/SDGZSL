...
parser.add_argument('--dataset', default='APY', help='dataset: CUB, AWA1, APY, FLO')
...
parser.add_argument('--lr', type=float, default=0.0001, help='learning rate to train the generater')
...
parser.add_argument('--dis', type=float, default=1, help='discriminator loss weight')
...
parser.add_argument('--manualSeed', type=int, default=3861, help='manual seed')
...
parser.add_argument('--NS_dim', type=int, default=128)
...
def train():
    ...
    for it in range(start_step, opt.niter+1):
        ...
        if it % opt.evl_interval == 0 and it > 15000:
            model.eval()
            ae.eval()
