...
parser.add_argument('--gen_nepoch', type=int, default=150, help='number of epochs to train for')
...
parser.add_argument('--dis', type=float, default=3, help='')

parser.add_argument('--classifier_lr', type=float, default=0.0015, help='learning rate to train softmax classifier')
parser.add_argument('--batchsize', type=int, default=64, help='input batch size')
parser.add_argument('--nSample', type=int, default=1800, help='number features to generate per class')
...
def train():
    ...
    for it in range(start_step, opt.niter+1):
        ...
        if it % opt.evl_interval == 0 and it > 20000:  #40000
            model.eval()
            ae.eval()
