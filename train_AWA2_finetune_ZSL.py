...
parser.add_argument('--gen_nepoch', type=int, default=40, help='number of epochs to train for')
...
parser.add_argument('--weight_decay', type=float, default=0, help='weight_decay')#1e-6
parser.add_argument('--dis', type=float, default=1, help='')
...
def train():
    ...
    for it in range(start_step, opt.niter+1):
        ...
        if it % opt.evl_interval == 0 and it > 0:  #40000
            model.eval()
            ae.eval()
